...
var licenseIds = map[string]string{} // file -> license ID mapping

func (p *Package) findLicense() (string, error) {
	if p.license != "" || p.Standard || p.ForTest != "" {
	...
	licenseId := licenseIds[licenseFile]
	if licenseId == "" {
		...
		if cov.Percent > 85 {
			licenseId = cov.Match[0].ID
		} else {
			return "", fmt.Errorf("license coverage too low for %s: %v", licenseFile, cov)
		}
		licenseIds[licenseFile] = licenseId
	}
